...
df.WebGrid = function WebGrid(sName, oParent){
    ...
    this._aOrigRowValues = null;    //  Orrigional values of the current row used for clearing
...
insertRow : function(tRow, bTop){
    ...
    if(tRow && tRow.aValues[0] === this.psCurrentRowID){
...
selectRow : function(sGotoRow, iRow, fOptHandler){
    ...
    if(sGotoRow === "new"){
        ...
        this._tNewRow = {
            aValues : new Array(this._aColumns.length + 1)
        };
        this._tNewRow.aValues[0] = "";
        for(i = 1; i < this._tNewRow.aValues.length; i++){
            if(this._aColumns[i - 1].peDataType === df.ciTypeBCD){
                this._tNewRow.aValues[i] = "0";
            }else{
                this._tNewRow.aValues[i] = "";
            }
        }
    ...
    df.WebGrid.base.selectRow.call(this, sGotoRow, iRow, function handleRowChangeGrid(bChanged){
        ...
        this._aOrigRowValues = (tRow && tRow.aValues.slice(0)) || null;
        ...
        if(bChanged && sPrevRowID === "" && iPrevRow >= 0){
            ...
            if(this._aCache[iPrevRow] && this._aCache[iPrevRow].aValues[0] === ""){
        ...
        if(iRow >= 0){
            for(iCol = 0; iCol < this._aColumns.length; iCol++){
                if(this._aColumns[iCol].get('pbChanged')){
                    this._aCache[iRow].aValues[iCol + 1] = this._aColumns[iCol].get('psValue');
            ...
            if(bChanged && this._aCache[iRow].aValues[0] === ""){
                this._aCache[iRow].aValues[0] = "new_" + (this._iNewCount++);
                this.psCurrentRowID = this._aCache[iRow].aValues[0];
            }
        }
    }
},
...
editCell : function(iRow, iCol){
    var eElem, eCell, oCol = this._aColumns[iCol], iCell;
...
unEditCell : function(iRow, iCol, bUpdate){
    ...
    if(bUpdate){
        ...
        this._aCache[iRow].aValues[iCol + 1] = sVal;
    }
    ...
    if(iRow >= this._iDispOffset && iRow < this._iDispOffset + this.getDisplaySize() && this._eTable.rows.length > iRow - this._iDispOffset && iCell >= 0){
        ...
        eCell.innerHTML = this.cellHtml(oCol, this._aCache[iRow].aValues[iCol + 1]);
        this._eEditCell = null;
...
updateRow : function(sRowID){
    ...
    if(this._aActionData.length > 0){
        ...
        if(iRow >= 0){
            ...
            if(sRowID === "" && tRow.aValues[0] === ""){
                tRow.aValues[0] = "new_" + (this._iNewCount++);
                if(this.psCurrentRowID === ""){
                    this.psCurrentRowID = tRow.aValues[0];
                }
            }
                ...
                if(iRow >= this._iDispOffset && iRow < this._iDispOffset + this.getDisplaySize()){
                    ...
                    this.loopCols(function(oCol, iCol, bLast, iCount){
                        ...
                        if(eRow.cells[iCount] !== this._eEditCell){
                            eRow.cells[iCount].innerHTML = this.cellHtml(oCol, tRow.aValues[iCol + 1]);
                        }
                    });
                }
            }
...
clearRow : function(sRowID){
    ...
    if(sRowID === this.psCurrentRowID){
        ...
        if(iRow >= 0){
            ...
            for(iCol = 0; iCol < this._aColumns.length; iCol++){
                // sDebug += ", '" + this._aCache[iRow].aValues[iCol + 1] + "'";
                if(this._aColumns[iCol].get('pbChanged')){
                    this._aColumns[iCol].set('psValue', (this._aOrigRowValues && this._aOrigRowValues[iCol + 1]) || "");
...
updateCurrentCell : function(oCol, sVal){
    ...
    if(this._tNewRow && this.psCurrentRowID === ""){
        ...
        this._tNewRow.aValues[iCol + 1] = sVal;
        ...
        if(iRow >= 0){
            ...
            if(this._eTable && iRow >= this._iDispOffset && iRow < this._iDispOffset + this.getDisplaySize()){
                ...
                if(iCell >= 0 && iCol !== this._iCurrentColumn && this._eTable.rows.length > iRow - this._iDispOffset){
                    this._eTable.rows[iRow - this._iDispOffset].cells[iCell].innerHTML = this.cellHtml(oCol, sVal);
                }
            }
        }
    }
...
onTableClick : function(oEvent){
    var eTarget, eElem, iCol = -1, iRow;
    
    eElem = eTarget = oEvent.getTarget();
        ...
        if(eElem.tagName === "TD"){
            ...
            if(iRow < this._aCache.length){
                ...
                if(iRow !== this.findRowByRowId(this.psCurrentRowID)){
                    ...
                    this.selectRow("row", iRow, function(bChanged){
                        if(bChanged){
                            if(this._aColumns[this._iCurrentColumn] instanceof df.WebColumnCheckbox && eTarget.tagName === "INPUT"){
                                this._aColumns[this._iCurrentColumn].tick();
                            }
                        }
                    });
                }else{
