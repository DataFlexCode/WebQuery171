...
df.WebObject = function WebObject(sName, oParent){
    ...
    this._aActionData = null;   //  Will contain custom data (two dimensional string array) if a client-action contains data
...
*/
getWebApp : function(){
    return (this instanceof df.WebApp && this) || (this._oParent && this._oParent.getWebApp && this._oParent.getWebApp()) || null;
},
...
@return True if the action is being sent, false if failed (due to other calls pending / sending).
*/
serverAction : function(sMethod, aOptParams, aOptData, fOptHandler, oOptEnv){
    ...
    //  Determine call mode
	oAction.eCallMode = this._oServerActionModes[sMethod] || df.cCallModeDefault;
    if(oAction.eCallMode === df.cCallModeProgress && this.psWaitMessage){
        oAction.sProcessMessage = this.psWaitMessage;
    }
	...
	return this.getWebApp().handleAction(oAction);
},
...
@client-action
*/
setActionMode : function(sMethod, eMode){
    this._oServerActionModes[sMethod] = parseInt(eMode, 10);
},
...
fire : function(sName, aOptParams, fOptHandler, oOptEnv){
    ...
    df.debug('Event: ' + sName + " " + df.sys.json.stringify(aOptParams || []) + " on " + this.getLongName() + "  (Call mode: " + this._oServerActionModes[sName] + " )");
