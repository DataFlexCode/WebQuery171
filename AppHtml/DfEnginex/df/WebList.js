...
insertRow : function(tRow, bTop){
    ...
    eRow.className = this.rowClass(tRow, bZebra);
    
    //  Insert cells
    this.loopCols(function(oCol, i){
        eCell = eRow.insertCell(-1);
        eCell.innerHTML = (tRow ? this.cellHtml(oCol, tRow.aCells[i]) : "&nbsp;");
        eCell.className = this.cellClass(oCol, tRow);
        
        eCell.title = (tRow && (tRow.aCells[i].sTooltip || tRow.aCells[i].sValue)) || "";
...
rowHtml : function(tRow, aHtml, bZebra){
    ...
    aHtml.push('<tr class="',  this.rowClass(tRow, bZebra),'">');
    
    //  Loop cells
    this.loopCols(function(oCol, i){
        sTooltip = (tRow && (tRow.aCells[i].sTooltip || tRow.aCells[i].sValue)) || "";
        sTooltip.replace('"', '&quot;');
        aHtml.push('<td class="', this.cellClass(oCol, tRow), '" title="', sTooltip, '">', (tRow ? this.cellHtml(oCol, tRow.aCells[i]) : '&nbsp;'), '</td>');
    });
...
@private
*/
cellClass : function(oCol, tRow){
    var aClasses =  [ ];
    
    aClasses.push(oCol._sCellClass);
    
    aClasses.push(oCol.pbEnabled ? "Web_Enabled" : "Web_Disabled");
...
cellHtml : function(oCol, tCell){
    ...
    tCell = tCell || { sValue : "", sTooltip : "", aOptions : [] };
...
selectRow : function(sGotoRow, iRow, fOptHandler, tOptSelectRowData){
    ...
    if(bContinue){
        if(!this.pbOfflineEditing){
            this.serverAction("ChangeCurrentRow", [ sGotoRow, sTargetRowID ], (tSelectRowData && this.serializeVT([ tSelectRowData ])) || null, handleRowChange, this);
            
            // TODO: Review invalid row system
        }else{
