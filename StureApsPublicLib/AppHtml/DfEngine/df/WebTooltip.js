df.WebTooltip = function(sName, oParent){
    df.WebTooltip.base.constructor.call(this, sName, oParent);
    
    this.prop(df.tString, "psMessage", "");
    this.prop(df.tInt, "piHideDelay", 6000);
    this.prop(df.tBool, "pbShowOnHover", true);
...
df.defineClass("df.WebTooltip", "df.WebObject", {

set_psMessage : function(sMessage){
    if(this._eElem){
        this._eElem.innerHTML = sMessage;
...
show : function(){
    ...
    if(!this._eElem){
         //  Create elem
        this._eElem = df.dom.create('<div class="WebTooltip ' + this.psCSSClass + '"></div>');
        this._eElem.innerHTML = this.psMessage;
        
        //  Insert
        this._eParentRef = eRef = df.sys.gui.findParentRef(this._oTarget);
        if(eRef){
...
_show : function(){
    if(this._eElem){
        ...
        df.dom.addClass(this._eElem, "WebTooltipVisible");
...
getTargetElem : function(){
    var eElem = this._oTarget._eElem;
    
    if(this._oTarget.getTooltipElem){
        eElem = this._oTarget.getTooltipElem() || eElem;
    }
...
_hide : function(){
    if(this._eElem){
        ...
        df.dom.removeClass(this._eElem, "WebTooltipVisible");
    }    
},
...
position : function(bOptFirstPos){
    ...
    if(eTool && eParent){
        ...
        iLeft = Math.round(eTarget.offsetWidth / 4);
        ...
        if(bOptFirstPos){
            ...
            if(eParent.clientHeight < (iTop + iHeight) && eParent.clientHeight > (iBottom + iHeight)){
                ...
                df.dom.addClass(this._eElem, "WebTooltip_Top");
